<template>
	<view class="page">
		...
		<view class="f__login">
			...
			<view class="desc">第一次登录会注册学籍绑定手机号，请按流程操作哦</view>
			<!-- 验证码登录 -->
			<view class="loginPhone">
				<view class="form-row">
					<input class="input" v-model="name" placeholder="请输入账号"
				...
				<view class="form-row u-flex">
					...
					<view class="u-p-l-6" @click="showPassword = !showPassword">
						<f-icon :name="showPassword?'xianshikejian':'yincangbukejian'" size="38"
							:color="showPassword?'#333':'#999'"></f-icon>
					</view>
				</view>
				...
				<view class="u-flex u-p-t-36" style="justify-content: center;">
					<view class="u-f-s-28" style="color: #666;" @click="onJump('/pages/user/forgetPassword')">忘记密码?
					</view>
					<view class="u-p-l-26 u-p-r-26">
						<u-line color="#666" direction="col" length="24rpx"></u-line>
					</view>
					<view class="u-f-s-28" style="color: #666;" @click="onJump('/pages/user/login')">验证码登录</view>
					<view class="u-p-l-26 u-p-r-26">
						<u-line color="#666" direction="col" length="24rpx"></u-line>
					</view>
					<view class="u-f-s-28" :style="{color:PrimaryColor}" @click="onJump('/pages/user/register')">注册账号
					</view>
				</view>
	...
	export default {
		...
		methods: {
			...
			onSubmit() {
				...
				uni.$u.http.post('/user/info/public/accountLogin', httpData).then(res => {
					this.setToken(res)
					this.getUserInfo()
					uni.showToast({
						title: '登录成功~',
			...
			getUserInfo() {
				uni.$u.http.post('/user/info/getPersonData', {}, {
					...
					console.log("123123", res.studentNumber);
					if (res.studentNumber === null || res.studentNumber === "" || res.studentNumber == undefined) {
						this.studentStatus = 0
					}
					console.log("this.studentS", this.studentStatus);
