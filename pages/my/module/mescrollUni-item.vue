<template>
	...
	<mescroll-uni ref="mescrollRef" @init="mescrollInit" :height="height" :down="downOption" @down="downCallback"
		...
		<!-- 数据列表 -->
		<!-- {{ height}} -->
		<course-list :list="newList"></course-list>
	</mescroll-uni>
</template>
...
<script>
	...
	export default {
		...
		watch: {
			newList(newVal) {
				// 监听 newList 的变化，将其赋值给子组件内部的 list
				this.newList = newVal;
				// 或者根据实际需求进行其他操作
			}
		},
		...
		methods: {
			...
			upCallback(page) {
				...
				uni.$u.http.post('/user/info/courseList', httpData, {
					custom: {
						load: false,
						auth: true
					}
					...
					this.newList = this.newList.concat(res); //追加新数据
				}).catch((err) => {
					//联网失败, 结束加载
