...
uni.$u.http.interceptors.request.use((config) => { // 可使用async await 做异步操作
	...
	// 根据custom参数中配置的是否需要token，添加对应的请求
...
againHttp.interceptors.request.use(config => {
	config.header.Authorization = store.state.token || (uni.getStorageSync('userInfo').token || '')
	return config
}, error => {
...
uni.$u.http.interceptors.response.use(async (response) => {
	...
	if (data.code === 0) {
		...
		if (custom.toast !== false) {
			uni.$u.toast(data.result || data.msg)
		}
		if (!custom.isFactory) {
			return data
		} else {
	...
	console.log(response, "error......sfsaaa")
	// 关闭加载动画
	store.commit("setLoadingShow", false);
